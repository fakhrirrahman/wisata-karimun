...
@section('title', 'Tambah Lokasi Terdekat')
@section('page-title', 'Tambah Lokasi Terdekat')

@section('content')
<div class="bg-white rounded-lg shadow p-6">
    <div class="mb-6">
        <h3 class="text-xl font-semibold text-gray-800">Wisata: {{ $wisata->nama }}</h3>
        <p class="text-sm text-gray-500">Data ini akan tampil di sidebar detail wisata.</p>
    </div>

    <form method="POST" action="{{ route('datawisata.nearby.store', $wisata->id) }}" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                >
                    ...
                    <option value="hotel" {{ old('kategori') == 'hotel' ? 'selected' : '' }}>Hotel</option>
                    <option value="restaurant" {{ old('kategori') == 'restaurant' ? 'selected' : '' }}>Tempat Makan</option>
                    <option value="service" {{ old('kategori') == 'service' ? 'selected' : '' }}>Layanan</option>
