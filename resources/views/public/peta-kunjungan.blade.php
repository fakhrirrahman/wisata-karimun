...
@section('title', 'Peta Kunjungan - Karimun')
...
@section('content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function getColorByJumlahKunjungan(v) {
                if (v > 25000) return '#a50f15'; // Sangat padat (>25000)
                if (v > 10000 && v <= 25000) return '#de2d26'; // Padat (10000-25000)
                if (v > 5000 && v <= 10000) return '#fb6a4a'; // Sedang (5000-10000)
                if (v >= 1 && v <= 5000) return '#fcae91'; // Sedikit (1-5000)
            ...
            const overlayMapsConfig = {
                ...
                "Heatmap Jumlah Kunjungan": heatmapGroup,
            ...
            legend.onAdd = function (map) {
                ...
                html += `<div class="legend-section-title">Heatmap Jumlah Kunjungan</div>`;
            ...
            async function fetchHeatmapData() {
                ...
                try {
                    const response = await fetch(`/api/kunjungan/heatmap?bulan=${bulan}&tahun=${tahun}`);
                    const data = await response.json();
                    
                    // Update global variable with fresh data from database
                    jumlahKunjunganPerKecamatan = data;
                    
                    // Re-render layers with new heatmap data
                    renderGeoJsonLayers();
                } catch (error) {
            ...
            // Listeners for month/year filter to fetch fresh DB data
                ...
                currentGeoJsonLayerHeatmap = L.geoJSON(geojsonData, {
                    ...
                    style: function (feature) {
                        ...
                        return {
                            ...
                            fillColor: getColorByJumlahKunjungan(visits),
                    ...
                    onEachFeature: function (feature, layer) {
                        const nameUpper = (feature.properties.NAMOBJ || '').toUpperCase();
                        const visits = jumlahKunjunganPerKecamatan[nameUpper] || 0;
                        layer.bindPopup(`<div style="text-align:center;color:#ef4444;"><strong>🔥 Heatmap Kec. ${feature.properties.NAMOBJ}</strong><br><span style="font-size:13px;font-weight:bold;color:#4b5563;">Jumlah Kunjungan: ${new Intl.NumberFormat('id-ID').format(visits)}</span></div>`);
